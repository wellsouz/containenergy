...
do
	...
	COMMAND="perf stat -r $RUNS -B -e task-clock,context-switches,cpu-migrations,page-faults,cycles,cycles:u,instructions,branches,branch-misses,power/energy-cores/,power/energy-pkg/,power/energy-ram/,power/energy-cores/,power/energy-pkg/,power/energy-ram/ --cgroup=$PERF_EVENT_CONTAINERCGROUP,$PERF_EVENT_CONTAINERCGROUP,$PERF_EVENT_CONTAINERCGROUP,$PERF_EVENT_CONTAINERCGROUP,$PERF_EVENT_CONTAINERCGROUP,$PERF_EVENT_CONTAINERCGROUP,$PERF_EVENT_CONTAINERCGROUP,$PERF_EVENT_CONTAINERCGROUP,$PERF_EVENT_CONTAINERCGROUP,$PERF_EVENT_CONTAINERCGROUP,$PERF_EVENT_CONTAINERCGROUP,$PERF_EVENT_CONTAINERCGROUP -a ./kvazaar_container_3.sh $i $i.container.hevc $THREADS $CPUSET $CONTAINER_NAME 2>/dev/null 2>> $DIR/$LOG"_"$FILENAME".container.stat" >> $DIR/$LOG"_"$FILENAME".container.stat""
	echo $COMMAND
	eval $COMMAND
	echo
