...
do
	...
	COMMAND="perf stat -r $RUNS -B -e task-clock,context-switches,cpu-migrations,page-faults,cycles,cycles:u,instructions,branches,branch-misses ./kvazaar_native.sh    $i $i.hevc $THREADS                         2>/dev/null 2>> $LOG"_"$FILENAME".native.stat"    >> $LOG"_"$FILENAME".native.stat""
	echo $COMMAND
	eval $COMMAND
	echo
	...
	COMMAND="perf stat -r $RUNS -B -e task-clock,context-switches,cpu-migrations,page-faults,cycles,cycles:u,instructions,branches,branch-misses --cgroup=$PERF_EVENT_CGROUP,$PERF_EVENT_CGROUP,$PERF_EVENT_CGROUP,$PERF_EVENT_CGROUP,$PERF_EVENT_CGROUP,$PERF_EVENT_CGROUP,$PERF_EVENT_CGROUP,$PERF_EVENT_CGROUP,$PERF_EVENT_CGROUP -a ./kvazaar_container.sh $i $i.hevc $THREADS $CPUSET $CONTAINER_NAME 2>/dev/null 2>> $LOG"_"$FILENAME".container.stat" >> $LOG"_"$FILENAME".container.stat""
	echo $COMMAND
	eval $COMMAND
	echo
