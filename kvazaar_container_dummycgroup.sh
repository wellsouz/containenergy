...
COMMAND="docker run --cgroup-parent=dockerkvazaardummy.slice --name $CONTAINER_NAME --cpuset-cpus=$CPUSET -i -a STDIN -a STDOUT -a STDERR kvazaar --threads $THREADS $INPUTRES $KVAZAAR_PARAMETERS -i - -o - < $INPUT > $OUTPUT"
echo $COMMAND
eval $COMMAND
